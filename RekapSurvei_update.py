...
def main():
    ...
    with st.form("form_input", clear_on_submit=True):
        ...
        nama_survei = col1.text_input("Nama Survei")
        lokasi = col2.text_input("Lokasi Survei")
        tanggal = col3.date_input("Tanggal Survei", value=datetime.today())

        col4, col5 = st.columns(2)
        alat = col4.text_input("Alat Survei")
        lampiran_files = col5.file_uploader("Lampiran (boleh lebih dari satu)", accept_multiple_files=True)

        hasil = st.text_area("Hasil Pengamatan / Catatan", height=120)

        st.markdown("---")
        st.write("📂 Pilih lokasi penyimpanan:")
        opsi_folder = st.radio("Opsi penyimpanan:", ["Gunakan folder utama", "Buat folder baru"], index=0)
        nama_folder_baru = ""
        selected_folder = None

        if opsi_folder == "Buat folder baru":
            nama_folder_baru = st.text_input("Nama Folder Baru")
        else:
            existing = [d for d in os.listdir(REKAP_DIR) if os.path.isdir(os.path.join(REKAP_DIR, d))]
            if existing:
                chosen = st.selectbox("Pilih folder existing:", ["(Pilih folder)"] + existing)
                if chosen != "(Pilih folder)":
                    selected_folder = chosen

        submitted = st.form_submit_button("💾 Simpan Data Hasil Survei")

        if submitted:
            if opsi_folder == "Buat folder baru" and nama_folder_baru:
                folder_tujuan = os.path.join(REKAP_DIR, nama_folder_baru)
                os.makedirs(folder_tujuan, exist_ok=True)
            elif opsi_folder == "Gunakan folder utama":
                folder_tujuan = REKAP_DIR
            else:
                folder_tujuan = os.path.join(REKAP_DIR, selected_folder) if selected_folder else REKAP_DIR
            ...
            st.success("✅ Data hasil survei tersimpan.")
            st.cache_data.clear()
            force_rerun()
    ...
    if not df.empty:
        ...
        st.markdown("### Daftar Rekapitulasi Hasil Survei")

        search = st.text_input("🔍 Cari data (ketik kata kunci, tekan Enter)")
        if search:
        ...
        entries_per_page = st.selectbox("Tampilkan jumlah baris:", [5, 10, 15], index=1)
